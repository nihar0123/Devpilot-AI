...
export default function SettingsPage() {
  ...
  useEffect(() => {
    void fetch("/api/team/context")
      ...
      .then((data) => {
        ...
        if (data?.usage) {
          setUsage({ used: data.usage.used || 0, limit: data.usage.limit || 50 });
        }
      })
      .catch(() => null);
  }, []);
  ...
  const avatarInitials = (session?.user?.name || session?.user?.email || "U")
    .split(/\s+/)
    .filter(Boolean)
    .slice(0, 2)
    .map((part) => part[0]?.toUpperCase() || "")
    .join("") || "U";
  const usagePercent = usage.limit ? Math.round((usage.used / usage.limit) * 100) : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog open={deleteOpen} onConfirm={deleteOrganization} onCancel={() => setDeleteOpen(false)} title="Delete Organization" description="This removes your workspace and all related resources. You will be signed out after confirmation." confirmLabel={deletingOrg ? "Deleting..." : "Delete Organization"} destructive />
    </div>
  );
}
